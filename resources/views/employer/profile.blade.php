...
<head>
    ...
    <title>Hiring - Employer Profile</title>
    ...
    <style>
        ...
        .top-nav { padding: 20px 5%; display: flex; justify-content: space-between; align-items: center; z-index: 10; position: relative; border-bottom: 1px solid var(--border-dark); background: rgba(19, 19, 26, 0.9); backdrop-filter: blur(10px); }
        ...
        .main-content { flex: 1; padding: 40px 5% 120px; max-width: 650px; margin: 0 auto; width: 100%; box-sizing: border-box; z-index: 5; position: relative; }
        ...
        .alert-success { background: rgba(74, 222, 128, 0.1); color: #4ade80; padding: 15px 20px; border-radius: 16px; margin-bottom: 25px; border: 1px solid rgba(74, 222, 128, 0.2); font-weight: 500;}
...
<body>
    ...
    <main class="main-content">
        ...
        <div class="profile-card">
            <h2 style="margin-top: 0; margin-bottom: 25px; color: var(--text-light); font-weight: 800; font-size: 24px;">Profil Perusahaan</h2>
            
            <form action="{{ route('profile.update') }}" method="POST" enctype="multipart/form-data">
                @csrf
                
                <!-- Tambahan Form Upload & Preview Foto Profil Perusahaan -->
                <div class="form-group" style="text-align: center; margin-bottom: 25px;">
                    <label style="display: block; text-align: center; margin-bottom: 12px;">Logo / Foto Perusahaan</label>
                    <div style="margin-bottom: 15px;">
                        @if(!empty($profile->profile_photo))
                            <img src="{{ asset('storage/' . $profile->profile_photo) }}" alt="Logo Perusahaan" style="width: 110px; height: 110px; border-radius: 50%; object-fit: cover; border: 3px solid var(--primary-orange); box-shadow: 0 4px 15px rgba(255,81,47,0.3);">
                        @else
                            <div style="width: 110px; height: 110px; border-radius: 50%; background: rgba(255,255,255,0.05); display: inline-flex; justify-content: center; align-items: center; border: 2px dashed var(--border-dark);">
                                <i class="fas fa-building" style="font-size: 45px; color: var(--text-muted);"></i>
                            </div>
                        @endif
                    </div>
                    <input type="file" name="profile_photo" accept=".jpg,.jpeg,.png" style="width: auto; max-width: 100%; display: inline-block; padding: 10px 15px; background: rgba(255,255,255,0.02); border-radius: 12px;">
                </div>

                <div class="form-group">
                    <label>Status Kebutuhan Perusahaan</label>
                    <select name="status">
                        <option value="active_searching" {{ ($profile->status ?? '') == 'active_searching' ? 'selected' : '' }}>Aktif Mencari Kandidat</option>
                        <option value="unactive" {{ ($profile->status ?? '') == 'unactive' ? 'selected' : '' }}>Sedang Tidak Mencari</option>
                    </select>
                </div>
                ...
                <div class="form-group">
                    <label>Jenis Industri / Bidang Perusahaan</label>
                    <input type="text" name="company_type" value="{{ $profile->company_type ?? '' }}" placeholder="Contoh: Teknologi, Kreatif, Tekstil">
                </div>

                <div class="form-group">
                    <label>Lokasi / Kantor Pusat</label>
                    <input type="text" name="location_employer" value="{{ $profile->location_employer ?? '' }}" placeholder="Contoh: Jakarta Pusat">
                </div>

                <div class="form-group">
                    <label>Upload Dokumen NPWP Perusahaan (PDF/JPG/PNG max 2MB)</label>
                ...
                <div class="form-group">
                    <label>Upload Dokumen NIB Perusahaan (PDF/JPG/PNG max 2MB)</label>
                    <input type="file" name="document_nib" accept=".jpg,.jpeg,.png,.pdf">
