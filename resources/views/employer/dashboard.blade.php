...
<head>
    ...
    <style>
        ...
        .logo { font-size: 28px; font-weight: 800; color: var(--primary-orange); letter-spacing: -1px; display:flex; align-items: baseline; gap: 8px; text-shadow: 0 4px 10px rgba(255,81,47,0.3);}
        ...
        .card-visual { height: 50%; background-color: #27272a; background-image: url('https://images.unsplash.com/photo-1560250097-0b93528c311a?q=80&w=600&auto=format&fit=crop'); background-size: cover; background-position: center; position: relative; }
        ...
        .rating-badge { position: absolute; top: 20px; right: 20px; background: rgba(0,0,0,0.7); backdrop-filter: blur(5px); padding: 8px 15px; border-radius: 20px; font-weight: 800; color: #f09819; border: 1px solid rgba(255,255,255,0.1); z-index: 10;}
...
<body>
    ...
    <div class="profile-alert" id="profileAlert" style="display: none;">
        <div><i class="fas fa-exclamation-triangle" style="margin-right: 8px;"></i> <strong>Verifikasi Perusahaan!</strong> Unggah NPWP dan Legalitas agar lowongan Anda diverifikasi sistem.</div>
        <a href="{{ url('/employer/profile') }}">Verifikasi Sekarang</a>
    </div>

    <div class="search-section" style="margin-top: 20px;">
        <div class="search-box">
            <input type="text" id="searchInput" placeholder="Cari keahlian kandidat...">
    ...
    <script>
        ...
        function renderTopCard() {
            ...
            if (currentCandidates.length === 0) {
                dynamicCardWrapper.innerHTML = `
                    <div style="text-align:center; color:var(--text-muted); margin-top:50%; font-family: inherit;">
                        ...
                        <p style="font-size:15px;">Belum ada kandidat baru.</p>
                    </div>`;
                return;
            }
            ...
            let history = applicant.job_history || 'No experience detailed.';

            let cardHTML = `
                <div class="card" id="topCard">
                    <div class="card-visual">
                        <div class="rating-badge">★ ${applicant.rating}</div>
                    </div>
                    
                    <div class="card-content">
                        <div class="company-label">${applicant.education || 'Education not set'}</div>
                        <h2 class="job-title">${name}</h2>
                        <div class="job-reqs">${history}</div>
                    </div>

                    <div class="card-actions">
                        <button class="btn-action btn-reject" onclick="triggerAction('reject', ${applicant.user_id}, ${dummyJobId})"><i class="fas fa-times"></i></button>
                        <button class="btn-action btn-accept" onclick="triggerAction('like', ${applicant.user_id}, ${dummyJobId})"><i class="fas fa-check"></i></button>
                    </div>
                </div>
            `;
            dynamicCardWrapper.insertAdjacentHTML('beforeend', cardHTML);
            initDrag(document.getElementById('topCard'), applicant.user_id, dummyJobId);
        }

        function triggerAction(action, applicantId, jobId) {
            const card = document.getElementById('topCard');
            if(!card) return;
            
            card.classList.add(action === 'like' ? 'swipe-out-right' : 'swipe-out-left');
            
            revealContent.innerHTML = action === 'like' ? '<span>💚</span> HIRE' : '<span>❌</span> REJECT';
        ...
        function initDrag(card, applicantId, jobId) {
            ...
            function onMove(e) {
                ...
                if (diffX > 0) {
                    revealContent.innerHTML = '<span>💚</span> HIRE';
            ...
            function onEnd() {
                ...
                if (diffX > threshold) {
                    triggerAction('like', applicantId, jobId);
                } else if (diffX < -threshold) {
                    triggerAction('reject', applicantId, jobId);
                } else {
