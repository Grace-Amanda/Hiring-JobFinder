...
<<<<<<< HEAD
            ...
            // Fix: coba parse langsung; jika gagal, coba unescape dulu.
            let parsed = null;
            const raw  = m.message;
            // Hanya coba parse jika terlihat seperti JSON object (diawali '{')
            if (raw && raw.trim().startsWith('{')) {
                try {
                    parsed = JSON.parse(raw);
                } catch (e1) {
                    try {
                        // Coba unescape backslash ganda lalu parse ulang
                        parsed = JSON.parse(raw.replace(/\\"/g, '"').replace(/\\\\/g, '\\'));
                    } catch (e2) {
                        parsed = null; // Bukan JSON valid, tampilkan sebagai teks biasa
                    }
                }
            }
            ...
            if (parsed && parsed.type === 'interview_slots') {
                // FIX BUG #2: Kirim swipeIdForBtn (snapshot) bukan curSwipeId
                // Bug lama: curSwipeId di dalam onclick bisa sudah berubah ke chat lain
                // saat polling async selesai, menyebabkan modal buka slot chat yang salah.
                renderSlotBubble(parsed, isMine, time, m.id, swipeIdForBtn);
...
<<<<<<< HEAD
                ...
                // Jika parsed adalah object tanpa type yang dikenali,
                // tampilkan pesan asli (bukan JSON.stringify ulang)
